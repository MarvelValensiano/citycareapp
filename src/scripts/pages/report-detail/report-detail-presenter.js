...
import { saveDataToReportStore, getDataByIdFromReportStore } from '../../utils/indexed-db';
import { saveBookmark, deleteBookmark, getBookmarkById } from '../../utils/bookmark-db'; // Pastikan impor ini ada dan benar
...
export default class ReportDetailPresenter {
  ...
  async showReportDetail() {
    ...
    if (navigator.onLine) {
      try {
        ...
        if (response.ok && response.data) {
          ...
          if (report) {
            // Pastikan report ada sebelum disimpan
            await saveDataToReportStore(report);
          }
        } else {
          console.warn(
            `Gagal mengambil detail laporan ${this.#reportId} dari API, mencoba dari cache. Pesan API:`,
            response.message,
          );
          fromCache = true;
        }
        ...
        console.error(
          `Kesalahan jaringan saat mengambil detail laporan ${this.#reportId}, mencoba dari cache:`,
          networkError,
        );
        fromCache = true;
      }
      ...
      console.log(
        `Offline: Mencoba mengambil detail laporan ${this.#reportId} dari IndexedDB cache...`,
      );
      fromCache = true;
    }
    ...
    if (fromCache) {
      ...
      if (report) {
        apiMessage = navigator.onLine
          ? `Gagal terhubung ke server, detail laporan ${this.#reportId} ditampilkan dari cache.`
          : `Anda offline, detail laporan ${this.#reportId} ditampilkan dari cache.`;
        console.log(`Detail laporan ${this.#reportId} berhasil dimuat dari IndexedDB cache.`);
      } else {
        apiMessage = navigator.onLine
          ? `Gagal mengambil detail dari server dan cache untuk laporan ${this.#reportId} kosong.`
          : `Anda offline dan detail laporan ${this.#reportId} tidak tersimpan di cache.`;
      }
    }
    ...
    if (report) {
      ...
      this.#view.populateReportDetailError(
        apiMessage || `Detail laporan dengan ID ${this.#reportId} tidak ditemukan.`,
      );
    }
  ...
  async handleSaveReport() {
    if (!this.#currentReportData || !this.#currentReportData.id) {
      ...
      console.warn(
        'Attempted to save report without currentReportData or id:',
  ...
  async handleRemoveReport() {
    if (!this.#currentReportData || !this.#currentReportData.id) {
      ...
      console.warn(
        'Attempted to remove report without currentReportData or id:',
        this.#currentReportData,
      );
      return;
    }
    try {
      ...
      alert(
        `Laporan "${this.#currentReportData.title || 'Tanpa Judul'}" telah dihapus dari simpanan.`,
      );
